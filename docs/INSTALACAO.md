...
## 6. Histórico mensal automático

Esta versão mantém somente o histórico do mês vigente visível.

Regra:

```txt
No 1º dia útil do mês seguinte, o histórico do mês anterior é fechado automaticamente.
```

Como funciona:

- a Vercel executa diariamente a rota `/api/fechamento-mensal` às 08h no horário de Brasília;
- a rota verifica se já chegou o 1º dia útil do mês;
- se chegou, o histórico visível é limpo e o sistema inicia o histórico do novo mês;
